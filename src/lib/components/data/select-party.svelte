<script lang="ts">
  ...
  import { PALETTES, get_initials, hash_name } from './utils';

  type CustomerOption = Awaited<ReturnType<typeof list_all_customers>>[number];
  ...
  let options = $derived(list_all_customers());
...
{#snippet optionSnippet({ option }: { option: CustomerOption })}
  {@const initials = get_initials(option.name)}
  {@const palette = PALETTES[hash_name(option.name)]}
  <div
    class="group relative inline-flex w-full max-w-full min-w-0 items-center gap-1.5 rounded-md px-1 py-0"
  >
    <span
...
>
  ...
  >
    ...
    {:else if !options.error && options.current}
      ...
      {:else}
        <span class="text-muted-foreground">Select party…</span>
      {/if}
    {:else}
      <div
