      ...
      {#each table.getHeaderGroups() as headerGroup (headerGroup.id)}
        ...
        >
          {#each headerGroup.headers as header (header.id)}
            ...
            <Table.Head
              class={cn(
                'h-8 truncate overflow-x-auto border-t-0 border-b-0 border-border bg-background px-0 text-left align-middle text-xs font-normal text-muted-foreground capitalize [&:has([role=checkbox])]:pr-0',
                'isolation-auto contain-none',
                headerClass
              )}
              style={createStyle({
        ...
        {#each table.getRowModel().rows as row (row.id)}
          ...
          >
            {#each row.getVisibleCells() as cell (cell.id)}
              ...
              <Table.Cell
                class={cn(
                  'min-h-8 truncate border-border px-3 py-1 align-middle text-sm font-medium text-ellipsis [&:has([role=checkbox])]:pr-0',
                  cellClass
                )}
                style={createStyle({
                  ...(cellStyle && cellStyle),
