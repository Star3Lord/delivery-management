<script lang="ts">
  ...
  import { get_recent_slips_by_product } from '$lib/api/delivery-slips.remote';
  import { Badge } from '$lib/components/ui/badge';
  import { Button } from '$lib/components/ui/button/index.js';
  import { Spinner } from '$lib/components/ui/spinner/index.js';
  import FileText from '@lucide/svelte/icons/file-text';
  import { vehicleIcon } from '../../vehicles/snippets.svelte';

  let { product_id }: { product_id: string } = $props();
</script>
...
<section>
  ...
  <ul class="mt-3 divide-y divide-border/40">
    <svelte:boundary>
      {#each await get_recent_slips_by_product( { product_id, limit: 20 } ) as delivery (delivery.id)}
        <li
            ...
            <div class="flex items-center gap-1">
              ...
              >
                {@render vehicleIcon({
                  ...
                  props: { class: 'size-3 text-muted-foreground/40' },
                })}
                {delivery.vehicle?.number_plate ?? '-'}
              </Badge>
            </div>
